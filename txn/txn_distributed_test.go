...
func TestDistributedTxnConsistencyIntegrate(t *testing.T) {
	testDistributedTxnConsistencyIntegrate(NewTestCase(t))
}

func TestDistributedTxnConsistencyIntegrateReadForWriteWaitNoWriteIntent(t *testing.T) {
	testDistributedTxnConsistencyIntegrate(NewTestCase(t).SetTxnType(types.TxnTypeReadForWrite).SetWaitNoWriteIntent())
}

func TestDistributedTxnConsistencyIntegrateRedis(t *testing.T) {
	testDistributedTxnConsistencyIntegrate(NewTestCase(t).SetDBType(types.DBTypeRedis))
}

func testDistributedTxnConsistencyIntegrate(ts *TestCase) {
	_ = flag.Set("logtostderr", fmt.Sprintf("%t", true))
	_ = flag.Set("v", fmt.Sprintf("%d", ts.LogLevel))

	for _, threshold := range []int{10000} {
		for i := 0; i < rounds; i++ {
			if !ts.True(testDistributedTxnConsistencyIntegrateOneRound(i,
				ts.SetStaleWriteThreshold(time.Millisecond*time.Duration(threshold)))) {
				ts.t.Errorf("testDistributedTxnConsistencyIntegrate failed @round %d", i)
				return
			}
		}
	}
...
func testDistributedTxnConsistencyIntegrateOneRound(round int, ts *TestCase) (b bool) {
	ts.t.Logf("testDistributedTxnConsistencyIntegrateOneRound @round %d\n"+
		"db type: \"%s\", txn type: \"%s\", wait no write intent: %v\n"+
		"stale threshold: %s, wound threshold: %s\n"+
		"go routine number: %d, txn number per go routine: %d, log level: %d", round,
	...
	if !ts.True(ts.genIntegrateTestEnv()) {
		return
	}
	defer ts.Close()
	...
	gAte := ts.txnServers[0].tm.kv.(*gate.Gate)
	if !ts.NotEqual(gAte.MustRoute(key1).ID, gAte.MustRoute(key2).ID) {
		return
	}
	...
	for i := 0; i < ts.GoRoutineNum; i++ {
		...
		go func(goRoutineIndex int) {
			...
			for round := 0; round < ts.TxnNumPerGoRoutine; round++ {
				if goRoutineIndex == 0 {
					ts.NoError(sc1.DoTransactionOfType(ctx, ts.TxnType, func(ctx context.Context, txn types.Txn) error {
						{
					...
					ts.NoError(sc2.DoTransactionOfType(ctx, ts.TxnType, func(ctx context.Context, txn types.Txn) error {
						{
							key1Val, err := txn.Get(ctx, key1, ts.ReadOpt)
							if err != nil {
								return err
			...
			ts.t.Logf("cost %v per round", time.Now().Sub(start)/time.Duration(ts.TxnNumPerGoRoutine))
		}(i)
	}
...

	{
		...
		if !ts.Equal(k1InitialValue-(ts.GoRoutineNum-1)*ts.TxnNumPerGoRoutine*delta+1*ts.TxnNumPerGoRoutine*key1ExtraDelta, value1) {
			return
		}
		...
		if !ts.Equal(k2InitialValue+(ts.GoRoutineNum-1)*ts.TxnNumPerGoRoutine*delta+1*ts.TxnNumPerGoRoutine*key2ExtraDelta, value2) {
			return
		}
	}
	{
...
func TestDistributedTxnConsistencyStandalone(t *testing.T) {
	...
	testDistributedTxnConsistencyIntegrate(NewTestCase(t).SetSmartClients(sc1, sc2))
}
